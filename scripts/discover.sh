#!/usr/bin/env bash
# discover.sh — Single-pass repository scanner for Copilot Brain Bootstrap
...
# Usage: bash scripts/discover.sh [project-dir]
...
if [[ "${BASH_SOURCE[0]}" != "${0}" ]]; then
  ...
  echo "   Wrong:  source scripts/discover.sh" >&2
  echo "   Right:  bash scripts/discover.sh [project-dir]" >&2
...
echo "# ============================================="
echo "# Copilot Brain Bootstrap — Discovery Results"
...
echo "# --- Pre-Flight ---"
emit "HAS_COPILOT_INSTRUCTIONS" "$([ -f ".github/copilot-instructions.md" ] && echo true || echo false)"
emit "HAS_GITHUB_DIR"           "$([ -d ".github" ] && echo true || echo false)"
emit "HAS_CONTEXT_DIR"          "$([ -d "context" ] && echo true || echo false)"
emit "HAS_LESSONS"              "$([ -f "context/tasks/lessons.md" ] && echo true || echo false)"
emit "HAS_SCRIPTS"              "$([ -d "scripts" ] && [ -f "scripts/discover.sh" ] && echo true || echo false)"

# Mode decision
# UPGRADE only if config exists AND has been bootstrapped.
#
# PROBLEM: {{PROJECT_NAME}} in copilot-instructions.md is an UNRELIABLE FRESH signal.
# When a user copies a new template over an existing bootstrapped config,
# the file gets the template version (with {{PROJECT_NAME}}) even though
...
#
#   1. context/tasks/COPILOT_ERRORS.md WITHOUT {{PROJECT_NAME}}
#      The template always ships COPILOT_ERRORS.md WITH {{PROJECT_NAME}}.
#      Once bootstrap runs, the placeholder is replaced → no placeholder = was bootstrapped.
#
#   2. context/tasks/lessons.md with real content beyond placeholder header
#
# Priority:
#   1. COPILOT_ERRORS.md bootstrapped (no {{...}})      → UPGRADE
#   2. Neither above, but {{PROJECT_NAME}} in copilot-instructions.md → FRESH
#   3. Neither above, no placeholder                     → UPGRADE

_errors_bootstrapped=false
if [ -f "context/tasks/COPILOT_ERRORS.md" ] && ! grep -q '{{PROJECT_NAME}}' "context/tasks/COPILOT_ERRORS.md" 2>/dev/null; then
  _errors_bootstrapped=true
fi

if [ -f ".github/copilot-instructions.md" ] || [ -d ".github" ] || [ -d "context" ]; then
  if [ "$_errors_bootstrapped" = "true" ]; then
    emit "MODE" "UPGRADE"
  elif [ -f ".github/copilot-instructions.md" ] && grep -q '{{PROJECT_NAME}}' ".github/copilot-instructions.md" 2>/dev/null; then
    emit "MODE" "FRESH"
  else
    emit "MODE" "UPGRADE"
  fi
...
if [ -f ".github/copilot-instructions.md" ] && [ -d "context/_examples" ] && [ -f "validate.sh" ] && [ -f "context/docs/DETAILED_GUIDE.md" ]; then
...
if [ "$PRIMARY_LANG" = "pl" ]; then
  ...
  if find . -maxdepth 3 -name '*.pm' -not -path '*/.git/*' -print0 2>/dev/null | xargs -0 grep -l 'Moose\|Moo\b' 2>/dev/null | head -1 | grep -q '.'; then FRAMEWORKS="${FRAMEWORKS}Moose,"; fi
  if find . -maxdepth 3 -name '*.pm' -not -path '*/.git/*' -print0 2>/dev/null | xargs -0 grep -l 'Catalyst\|Mojolicious\|Dancer' 2>/dev/null | head -1 | grep -q '.'; then FRAMEWORKS="${FRAMEWORKS}Perl-web-fw,"; fi
fi
...
if [ "$PRIMARY_LANG" = "swift" ] && find . -maxdepth 5 -name '*.swift' -not -path '*/.git/*' -print0 2>/dev/null | xargs -0 grep -l 'import SwiftUI' 2>/dev/null | head -1 | grep -q '.'; then
...
if find . -maxdepth 3 -name 'Application.yaml' -print0 2>/dev/null | xargs -0 grep -l 'argoproj.io' 2>/dev/null | head -1 | grep -q '.' 2>/dev/null; then FRAMEWORKS="${FRAMEWORKS}ArgoCD,"; fi
...
# ─── Copilot Tools Detection ─────────────────────────────────────

echo "# --- Tools ---"

# graphify: pip package + global skill
HAS_GRAPHIFY="false"
...
# .serena config present (LSP integration active)
HAS_SERENA_CONFIG="false"
[ -d ".serena" ] && HAS_SERENA_CONFIG="true"

# Copilot instructions customization level
# Detect how far along the Copilot config is from default template
HAS_CUSTOM_INSTRUCTIONS="false"
if [ -f ".github/copilot-instructions.md" ] && ! grep -q '{{PROJECT_NAME}}' ".github/copilot-instructions.md" 2>/dev/null; then
  HAS_CUSTOM_INSTRUCTIONS="true"
fi

# VS Code Copilot Chat extension presence (workspace .vscode/settings.json)
HAS_VSCODE_COPILOT="false"
if grep -q 'github.copilot' ".vscode/settings.json" 2>/dev/null \
   || grep -q 'github.copilot' ".vscode/extensions.json" 2>/dev/null; then
  HAS_VSCODE_COPILOT="true"
fi
...
emit "HAS_SERENA_CONFIG" "$HAS_SERENA_CONFIG"
emit "HAS_CUSTOM_INSTRUCTIONS" "$HAS_CUSTOM_INSTRUCTIONS"
emit "HAS_VSCODE_COPILOT" "$HAS_VSCODE_COPILOT"
