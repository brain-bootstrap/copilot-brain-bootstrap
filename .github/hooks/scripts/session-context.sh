...
if [[ -f "context/tasks/todo.md" ]]; then
  ...
  CONTEXT+="**Open todos:** ${OPEN_TODOS} (see \`context/tasks/todo.md\`)\n"
  if [[ "${OPEN_TODOS}" -gt 0 ]]; then
    CONTEXT+="\n**Pending items:**\n"
    while IFS= read -r line; do
      CONTEXT+="  ${line}\n"
    done < <(grep '^\- \[ \]' context/tasks/todo.md 2>/dev/null | head -5 || true)
  fi
  CONTEXT+="\n"
fi
...
if [[ -f "context/tasks/lessons.md" ]]; then
  LESSON_LINES=$(grep -v '^#' context/tasks/lessons.md 2>/dev/null | grep -v '^$' | tail -10 | head -5 || true)
...
if [[ -f "context/tasks/COPILOT_ERRORS.md" ]]; then
  ERROR_HEADERS=$(grep '^## ERROR-' context/tasks/COPILOT_ERRORS.md 2>/dev/null | tail -3 || true)
  if [[ -n "${ERROR_HEADERS}" ]]; then
    CONTEXT+="**Active error patterns:**\n${ERROR_HEADERS}\n\n"
  fi
fi

CONTEXT+="⚡ First steps: read \`context/tasks/lessons.md\` + \`context/architecture.md\` + \`context/rules.md\`\n"
CONTEXT+="⚡ NEVER \`git push\` autonomously — present summary, wait for confirmation\n"
CONTEXT+="⚡ Temp files → \`./context/tasks/\` — never \`/tmp/\`\n"
