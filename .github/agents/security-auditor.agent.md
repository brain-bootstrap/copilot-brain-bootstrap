...
```bash
# Secrets scan
grep -rn 'password\|secret\|api_key\|token\|credential' --include='*.py' --include='*.ts' --include='*.env*' . 2>/dev/null | head -30
...
# Dangerous patterns
grep -rn 'eval(\|exec(\|subprocess.call(' --include='*.py' . 2>/dev/null | head -20
grep -rn 'innerHTML\|dangerouslySetInnerHTML' --include='*.ts' --include='*.tsx' . 2>/dev/null | head -20
```
